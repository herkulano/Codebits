Project = Ext.extend(Object, {
    ...
    copyResources : function() {
        ...
        this.get('resources').forEach(function(resource) {
            var filters = resource.filters,
                ...
                dest = Platform.isUnix ? (resource.dest || resource.src) : resource.dest;
                
            // TODO: Implement filters
            if (this.builder.get('verbose')) {
                Logger.log('  + ' + resource.src);
            }
            dest = dest || '';
            Fs.copy(srcDir, this.get('deployDir') + Fs.sep + dest);
        }, this);
    },
    ...
    compressTarget : function(target) {
        if (this.builder.get('nocompress')) {
            return;
        }
